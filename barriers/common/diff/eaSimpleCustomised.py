...
def batch_fitness_simulation(population, max_batch):
    ...
    for individual in population:
        if get_fitness(dictionary, individual) == False:
            if len(to_batch_up) == 0:
                to_batch_up.append(individual)
            else:
                for i in range(len(to_batch_up)):
                    if np.array_equal(to_batch_up[i],individual):
                        break
                    elif i==(len(to_batch_up)-1):
                        to_batch_up.append(individual)
    new_fitnesses = []
    ...
    for individual in initial_population:
        fitness = get_fitness(new_dictionary, individual)
